...
void heapify(int arr[], int n, int i)
{
	...
	if(l<n && arr[l] > arr[lar])	//if left child is larger than root
		lar = l;
	if(r<n && arr[r] > arr[lar])	//if right child is larger than root
		lar = r;
		
	if(lar != i)					//if lar is not root
	{
		swap(&arr[i],&arr[lar]);
		heapify(arr,n,lar);			//recursive call for affected subtree
	}
}
